...
struct SgNodeInterp2D_type {
    ...
    void computeWeights() {
        ...
        for (size_t dstNodeIndex = 0; dstNodeIndex < this->dstNumPoints; ++dstNodeIndex) {
            ...
            // start search
            int ier = this->findTarget(targetPoint, dIndices);
            ...
            if (ier == 0) {
                ...
                SgSideIterator_type sit(2); // 2D
                sit.reset();
                int stop = 0;
                ier = 1;
                while ((stop != 1) && (ier != 0)) {
                    std::vector<int> side = sit.get();
                    for (size_t i = 0; i < NDIMS_2D_TOPO; ++i) {
                        dIndices[i] = (side[i] + 1) * this->srcCellDims[i] / 2.0;
                    }
                    ier = this->findTarget(targetPoint, dIndices);
                    stop = sit.next();
                }
...
private:
     ...
     * @param dIndices (in/out) initial/final index position
