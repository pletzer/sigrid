/**
 * A class that finds the area overlaps between two structured grids
 */
 
#ifndef SG_FIND_AREA_OVERLAPS_H
#define SG_FIND_AREA_OVERLAPS_H
...
struct SgFindAreaOverlaps_type {

 	// linear solver
 	SgLinearSolve_type* slvr;

 	// the source grid curvilinear coordinate points as ndims flat arrays
 	std::vector<double> srcCoords[2];

 	// the destinations grid curvilinear coordinate points as ndims flat arrays
 	std::vector<double> dstCoords[2];

 	// used to get the cell flat index
 	std::vector<size_t> srcCellProdDims;

 	std::vector<size_t> dstCellProdDims;

 	// the number of grid points for each dimension
 	std::vector<size_t> srcNodeDims;
 	std::vector<size_t> dstNodeDims;
