...
struct SgOctreElement {
    std::vector<double> xmins;
    std::vector<double> xmaxs;
    SgOctreElement(xLo, xHi) : xmins(xLo), xmaxs(xHi) {
    }
...
struct SgOctreePoints_type {
    ...
    std::map< std::vector<size_t>, SgOctreElement > octree;
...
SgOctreePoints_type(size_t numLevels, size_t ndims, const std::vector<double>& points) {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        size_t dim = i % ndims;
        double p = points[i];
        xmins[dim] = (p < xmins[dim]? p: xmins[dims]);
        xmaxs[dim] = (p > xmaxs[dim]? p: xmaxs[dims]);
    }
    ...
    for (size_t level = 0; level < numLevels; ++level) {
        ...
        for (size_t iBox = 0; iBox < std::pow(2, ndims); ++iBox) {
            ...
            SgOctreElement elem(lo, hi);
            this->octree.insert(key, elem);
            }
        }
    }
    ...
    for (size_t i = 0; i < numPoints; ++i) {
        std::vector<size_t> key = this->getKey(&points[i*ndims]);
...
const std::vector<double>& getLo(const std::vector<size_t>& key) {
    return this->octree[key].xmins;
}

const std::vector<double>& getHi(const std::vector<size_t>& key) {
    return this->octree[key].xmaxs;
}

const std::vector<size_t>& getPointIndices(const std::vector<size_t>& key) {
    return this->octree[key].indices;
}

private:

    std::vector<size_t> getKey(const double* pt, size_t lev = this->nlevs) {
        // normalize
        std::vector<double> x(point, point + this->ndims);
        for (size_t j = 0; j < this->ndims; ++j) {
            double x[j] = (pt[j] - this->xmins[i])/(this->xmaxs[j] - this->xmins[j]); // should not be zero
        }
